    ...
    var render = function render(res) {
      ...
      for (var j = 0, len2 = res.list.length; j < len2; j++) {
        ...
        for (var i = 0, len = data.link.length; i < len; i++) {
          ...
          //var minSrc = 'https://raw.githubusercontent.com/your_github/test_album/master/min_photos/' + data.link[i];
          var minSrc = 'https://raw.githubusercontent.com/LiuZX886/MyAlbum/master/min_photos/' + data.link[i];
    
          //var minSrc = 'https://raw.githubusercontent.com/your_github/test_album/master/min_photos/' + data.link[i];
          var src = 'https://raw.githubusercontent.com/LiuZX886/MyAlbum/master/photos/' + data.link[i];
         
          var type = data.type[i];
          var target = src + (type === 'video' ? '.mp4' : '.jpg');


          liTmpl += '<li class="photo-item" itemprop="associatedMedia" itemscope="" itemtype="http://schema.org/ImageObject">\
      ...
      // 2. 在HTML插入后，初始化瀑布流
      var grid = document.querySelector('.img-box-ul'); // 获取图片列表的容器
      var msnry = new Masonry(grid, {
          itemSelector: '.photo-item', // 告诉Masonry哪个是我们的图片项
          columnWidth: '.photo-item',  // 让Masonry以一个图片项的宽度作为列宽基准
          gutter: 10,                  // 设置项目之间的水平间距为10像素
          percentPosition: true,       // **关键**：因为我们的CSS用的是百分比宽度，这个必须开启
          horizontalOrder: true,       // 让照片尽可能按从左到右的顺序排列，更自然
          fitWidth: true,              // **推荐**：让整个相册容器宽度自适应并支持居中
          transitionDuration: '0.4s'   // 设置一个平滑的动画过渡效果
      });
